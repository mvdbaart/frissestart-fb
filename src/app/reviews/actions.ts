...
const reviewsSeedData: Omit<Review, 'id' | 'createdAt'>[] = [
  // Kopieer hier de inhoud van je reviews_20250530_115234.json bestand
  // Voorbeeld (vervang dit met je volledige data):
  {
    "date": "2024-11-30",
    "rating": 9,
    "subratings": {
...
export async function seedDatabaseWithJsonReviews(): Promise<{ success: boolean; message: string; count?: number }> {
  try {
    // Optioneel: check of de database al data bevat om duplicaten te voorkomen
    // Dit is een simpele check, voor robuustere seeding zou je per item moeten checken.
    const reviewsCollection = collection(db, 'reviews');
    const snapshot = await getDocs(query(reviewsCollection, where("reviewer_name", "!=", ""))); // Een simpele query om te zien of er data is
    if (!snapshot.empty && snapshot.docs.length >= reviewsSeedData.length * 0.8) { // *0.8 om kleine verschillen toe te staan
      // Database lijkt al geseed te zijn of bevat aanzienlijke data
      // Je zou hier kunnen kiezen om de bestaande data te verwijderen, of de actie te stoppen.
      // Voor nu, stop de actie om duplicaten te minimaliseren.
      console.log('Database lijkt al gevuld te zijn. Seeding overgeslagen.');
      return { success: false, message: 'Database lijkt al gevuld te zijn. Seeding niet uitgevoerd om duplicaten te voorkomen.' };
    }
    ...
    for (const review of reviewsSeedData) {
      ...
      try {
        createdAtTimestamp = Timestamp.fromDate(new Date(date));
      } catch (e) {
        console.warn(`Ongeldige datumformaat voor review "${title}": ${date}. Huidige datum wordt gebruikt.`);
        createdAtTimestamp = Timestamp.now();
      }

      const reviewDocRef = collection(db, 'reviews').doc(); // Genereer een nieuw document ID

      const subratingsToStore: SubRatings = {};
      if (subratings) {
      ...
      batch.set(reviewDocRef, {
        date, // Bewaar de originele datumstring
        rating, // Schaal 1-10
        ...
        recommended: recommended !== undefined ? recommended : rating >= 7, // Bepaal 'recommended' als niet aanwezig
    ...
    await batch.commit();
    console.log(`Successfully seeded ${importedCount} reviews into Firestore.`);
    return { success: true, message: `Succesvol ${importedCount} reviews geïmporteerd in de database.`, count: importedCount };
  } catch (error) {
    console.error("Error seeding database with JSON reviews:", error);
